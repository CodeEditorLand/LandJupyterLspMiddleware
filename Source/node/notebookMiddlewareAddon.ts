...
} from "vscode-languageclient/lib/common/codeLens";
...
} from "vscode-languageclient/lib/common/rename";
 ...
 */
export class NotebookMiddlewareAddon
	implements protocol.Middleware, vscode.Disposable
{
	...
	constructor(
		...
		this.converter = concat.createConverter(getNotebookHeader, () =>
			os.platform()
		);
	...
	public stopWatching(notebook: vscode.NotebookDocument): void {
		...
		if (client && notebook.cellCount > 0) {
			...
			notebook.getCells().forEach((c) => {
				this.converter.handleClose({
					textDocument: { uri: c.document.uri.toString() },
				});
			});
		}
	}
	...
	public async didChange(event: vscode.TextDocumentChangeEvent) {
		...
		if (
			isNotebookCell(event.document.uri) &&
			client &&
	...
	public async didOpen(
		...
		_next: (ev: vscode.TextDocument) => Promise<void>
	) {
		// We need to talk directly to the language client here.
		const client = this.getClient();
	...
	public async didClose(
		document: vscode.TextDocument,
		_next: (ev: vscode.TextDocument) => void
	): Promise<void> {
		// We need to talk directly to the language client here.
		const client = this.getClient();
	...
	public didSave(
		event: vscode.TextDocument,
		next: (ev: vscode.TextDocument) => Promise<void>
	) {
		return next(event);
	}
	...
	public willSaveWaitUntil(
		event: vscode.TextDocumentWillSaveEvent,
		next: (
			ev: vscode.TextDocumentWillSaveEvent
			...
			if (
				...
				client.code2ProtocolConverter
					...
					.then((protocolDiagnostics) => {
						...
						const newDiagMapping =
							this.converter.toNotebookDiagnosticsMap(
								uri.toString(),
								protocolDiagnostics
							);
						...
						[...newDiagMapping.keys()].map(async (k) =>
							next(
								vscode.Uri.parse(k),
								await client.protocol2CodeConverter.asDiagnostics(
									newDiagMapping.get(k)!
								)
							)
						);
					});
	...
	public async provideDocumentSemanticTokens(
		...
		if (this.shouldProvideIntellisense(document.uri) && client) {
			...
			const notebookResults = this.converter.toNotebookSemanticTokens(
				documentId,
				result
			);
	...
	public async provideLinkedEditingRange(
		...
		if (this.shouldProvideIntellisense(document.uri) && client) {
			...
			// Then convert from protocol back to vscode types
			const notebookResults =
				this.converter.toNotebookLinkedEditingRanges(
					documentId,
					result
				);
	...
	private asTextDocumentIdentifier(
