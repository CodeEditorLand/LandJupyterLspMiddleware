...
export function mockTextDocument(
	uri: vscode.Uri,
	languageId: string,
	source: string[]
) {
...
export function withTestNotebook(
	...
	const notebookCells = cells.map((cell, index) => {
		const cellUri = uri.with({
			fragment: `ch${index.toString().padStart(7, "0")}`,
...
export const EXTENSION_ROOT_DIR_FOR_TESTS = path.join(
	__dirname,
	"..",
	"..",
	"..",
 ...
 */
const pendingCellUpdates = new WeakMap<
...
export function clearPendingChainedUpdatesForTests() {
	const editor: vscode.NotebookEditor | undefined =
		vscode.window.activeNotebookEditor;
	if (editor?.document) {
		pendingCellUpdates.delete(editor.document);
	}
}
export async function insertMarkdownCell(
	source: string,
	...
	await chainWithPendingUpdates(activeEditor.document, (edit) => {
		...
		edit.set(activeEditor.document.uri, [
			vscode.NotebookEdit.replaceCells(
				new vscode.NotebookRange(startNumber, startNumber),
				[cellData]
			),
		]);
	});
...
export async function insertCodeCell(
	source: string,
	options?: { language?: string; index?: number }
) {
	const activeEditor = vscode.window.activeNotebookEditor;
	if (!activeEditor) {
		throw new Error("No active editor");
...
export async function createTemporaryFile(options: {
	...
	return {
		file: tempFile,
		dispose: () => swallowExceptions(() => fs.unlinkSync(tempFile)),
	};
}
	...
	const closeWindowsImplementation = (timeout = 2_000) => {
		return new Promise<void>((resolve, reject) => {
			...
			const timer = setTimeout(
				() => reject(new CloseEditorsTimeoutError()),
				timeout
			);
			vscode.commands
...
function isANotebookOpen() {
	/* eslint-disable */
	if (
		Array.isArray(vscode.window.visibleNotebookEditors) &&
		vscode.window.visibleNotebookEditors.length
	) {
		return true;
	}
...
export async function closeActiveWindows(
	disposables: vscode.Disposable[] = []
): Promise<void> {
...
export async function closeNotebooksAndCleanUpAfterTests(
	disposables: vscode.Disposable[] = []
) {
	clearOutputMessages();
 ...
 */
export async function waitForCellExecutionToComplete(
...
function getCellOutputs(cell: vscode.NotebookCell) {
	return cell.outputs.length
		? cell.outputs
				.map((output) => output.items.map(getOutputText).join("\n"))
				.join("\n")
		: "<No cell outputs>";
}
...
function hasTextOutputValue(
	output: vscode.NotebookCellOutputItem,
	value: string,
	isExactMatch = true
) {
	if (
	...
	try {
		const haystack = Buffer.from(output.data as Uint8Array).toString(
			"utf8"
		);
		return isExactMatch
...
export function assertHasTextOutputInVSCode(
	...
	const result = cell.outputs[index].items.some((item) =>
		hasTextOutputValue(item, text, isExactMatch)
	);
	assert.isTrue(
		result,
		`${text} not found in outputs of cell ${cell.index} ${cell.outputs[
			index
		].items
			.map((o) =>
				o.data ? Buffer.from(o.data as Uint8Array).toString("utf8") : ""
			)
			.join(" ")}`
	);
	return result;
...
export async function waitForTextOutput(
	...
	await waitForCondition(
		...
		timeout,
		() =>
			`Output does not contain provided text '${text}' for Cell ${
				cell.index + 1
...
class NerfedExecuteCommandFeature
	implements DynamicFeature<ExecuteCommandRegistrationOptions>
{
	private readonly _commands: Map<string, vscode.Disposable[]> = new Map<
		string,
		vscode.Disposable[]
	>();
...
function trackNotebookCellMovement(
	middleware: NotebookMiddleware
): vscode.Disposable {
...
async function startLanguageServer(
	...
	const modulePath = (await fs.pathExists(nonBundlePath))
		? nonBundlePath
		: bundlePath;
	...
	const client = new vslc.LanguageClient(
		"lsp-middleware-test",
		serverOptions,
		clientOptions
	);
	...
	const features: (StaticFeature | DynamicFeature<any>)[] = (client as any)
		._features;
...
export async function createLanguageServer(
	...
	assert.isOk(
		python,
		"Python extension not installed, test suite cannot run"
	);
	await python?.activate();
	const pythonExports = python?.exports;
	const pythonPath =
		pythonExports?.settings.getExecutionDetails().execCommand[0] ||
	...
	assert.isOk(
		pylance,
		"Pylance extension not installed, test suite cannot run"
	);
