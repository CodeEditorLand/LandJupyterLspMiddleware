...
suite("Notebook tests", function () {
	...
	test("Add some cells and get completions", async () => {
		...
		assert.ok(
			items.find((item) =>
				typeof item === "string"
					? item.includes("bit_length")
					: item.label.includes("bit_length")
			)
		);
		assert.ok(
			items.find((item) =>
				typeof item === "string"
					? item.includes("to_bytes")
					: item.label.includes("to_bytes")
			)
		);
	});
	test("Edit a cell and make sure diagnostics change", async () => {
		...
		assert.isEmpty(
			diagnostics,
			"No diagnostics should be found in the first cell"
		);
		...
		edit.replace(
			cell.document.uri,
			new Range(new Position(0, 7), new Position(0, 10)),
			"system"
		);
	...
	test("Insert cells in the middle", async () => {
		...
		assert.ok(
			diagnostics,
			"Import system should generate a diag error on middle cell"
		);
		assert.ok(
			diagnostics.find((item) => item.message.includes("system")),
			"System message not found"
	...
	test("Replace contents of cell", async () => {
		...
		edit.replace(
			cell.document.uri,
			new Range(new Position(0, 7), new Position(2, 5)),
			"stuff"
		);
		await workspace.applyEdit(edit);
	...
	test("Move cell up and down (and make sure diags move with it)", async () => {
		await insertCodeCell("import sys\nprint(sys.executable)");
		let cell2 = await insertCodeCell(
			"import system\nprint(sys.executable)"
		);
		await insertCodeCell("import sys\nprint(sys.executable)");
		...
		assert.isEmpty(
			diagnostics,
			`Diagnostics should be empty after moving ${JSON.stringify(
				diagnostics
			)}`
		);
	...
	test("Move cell with variable up and down (and make sure diags appear)", async () => {
		...
		assert.ok(
			diagnostics,
			"Moving variable down should cause diags to show up"
		);
		assert.ok(
	...
	test("Add some errors with markdown and delete some cells", async () => {
		...
		assert.ok(
			diagnostics,
			"Import fuzzbaz should generate a diag error on middle cell"
		);
		assert.ok(
	...
	test("Make sure diags are skipped when not allowing", async function () {
		...
		assert.isEmpty(
			diagnostics,
			`No diagnostics should be found in the third cell ${JSON.stringify(
				diagnostics
			)}`
		);
	});
	test("Delete a cell with a variable in it", async () => {
		...
		assert.isNotEmpty(
			diagnostics,
			"Deleting sys should create diagnostics"
		);
	});
